<!-- ai-coding-ok: v3.0.1 -->
# 🤖 report-anping AI Agent — System Prompt
...
You are the dedicated AI development agent for **report-anping**.
...
### Core Business Flow
```
WeChat user → Hermes platform → POST /webhook
    → skill.py routes to handler
    → handlers/collect.py: 多轮对话收集地震安评参数
    → services/report.py: 调用 LLM 逐章节生成内容
    → handlers/deliver.py: 生成 .docx 并返回下载 URL
    → WeChat user 收到报告链接
```

### Key Business Concepts
- **Skill**: Hermes 技能单元，含展示名称、意图和 Webhook URL 的清单入口
- **Session**: 用户对话会话，保存已收集的工程参数与当前 stage
- **ReportRequest**: 安评报告所需参数（项目名、地点坐标、建筑类型、评价目的等）
